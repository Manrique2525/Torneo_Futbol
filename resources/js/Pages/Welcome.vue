...
// Transformar planes del backend al formato que usa la vista
const plansData = computed(() => {
    if (!props.planes || props.planes.length === 0) {
        return {
            mensual: [],
            anual: []
        };
    ...
    const mensual = props.planes.map(plan => ({
        id: plan.id,
        nombre: plan.name,
        slug: plan.slug,
        descripcion: plan.description,
        precio: plan.monthly_price, // Precio mensual
        popular: plan.is_featured,
        features: getFeaturesList(plan),
        limitaciones: getLimitationsList(plan)
    }));

    const anual = props.planes.map(plan => ({
        id: plan.id,
        nombre: plan.name,
        slug: plan.slug,
        descripcion: plan.description,
        precio: plan.annual_price, // Precio anual completo
        precioMensualEquivalente: Math.round(plan.annual_price / 12), // Para mostrar el ahorro
        precioOriginalMensual: plan.monthly_price,
        popular: plan.is_featured,
        features: getFeaturesList(plan),
        limitaciones: getLimitationsList(plan)
    }));
...
// Obtener lista de características del plan
function getFeaturesList(plan) {
    const features = [];

    // Características por límites
    features.push(`${getLimitDisplay(plan.max_tournaments)} torneo(s) activo(s)`);
    features.push(`${getLimitDisplay(plan.max_teams)} equipo(s)`);
    features.push(`${getLimitDisplay(plan.max_players)} jugador(es)`);
    features.push(`${getLimitDisplay(plan.max_users)} usuario(s) admin`);
    features.push(`${getLimitDisplay(plan.max_fields)} cancha(s)`);
    features.push(`${getLimitDisplay(plan.max_referees)} árbitro(s)`);
    
    // Almacenamiento
    if (plan.storage_mb > 0) {
        features.push(plan.storage_mb === -1 
            ? 'Almacenamiento ilimitado' 
            : `${plan.storage_mb} MB de almacenamiento`);
    }
    ...
    if (plan.has_reports) {
        features.push('Reportes financieros y estadísticos');
    }
    
    if (plan.has_mobile_app) {
        features.push('App móvil incluida');
    }
    
    if (plan.has_whatsapp) {
        features.push('Notificaciones por WhatsApp');
    }
    
    if (plan.has_streaming) {
        features.push('Streaming integrado');
    }
    
    if (plan.has_api_access) {
        features.push('API completa para integraciones');
    }
    ...
    switch (plan.support_level) {
        case 'priority':
            ...
            break;
        case 'dedicated':
            features.push('Soporte dedicado 24/7');
            break;
        default:
...
// Obtener lista de limitaciones del plan
function getLimitationsList(plan) {
    const limitations = [];

    // Si es Enterprise (o tiene todo), no tiene limitaciones
    if (plan.slug === 'enterprise' || plan.max_tournaments === -1) {
        return limitations;
    }

    // Limitaciones basadas en características faltantes
    if (!plan.has_mobile_app) {
        limitations.push('Sin app móvil');
    }
    
    if (!plan.has_streaming) {
        limitations.push('Sin streaming');
    }
    
    if (!plan.has_api_access) {
        limitations.push('Sin acceso a API');
    }

    // Límites que podrían ser restrictivos
    if (plan.max_tournaments === 2) {
        limitations.push('Máximo 2 torneos simultáneos');
    }

    return limitations;
}

// Mostrar valor de límite (Ilimitado si es -1)
function getLimitDisplay(value) {
...
const calculateAnnualSavings = computed(() => {
    if (!props.planes || props.planes.length === 0) return '0';
    
    // Usar el plan Pro como referencia (el más común)
    const proPlan = props.planes.find(p => p.slug === 'pro');
    if (!proPlan) return '17';
    
    const monthlyOriginal = proPlan.monthly_price;
    const monthlyAnnual = proPlan.annual_price / 12;
    const savingsPercent = ((monthlyOriginal - monthlyAnnual) / monthlyOriginal * 100);
...
<template>
    ...
    >
        ...
        <nav class="fixed top-0 left-0 right-0 z-50 border-b border-gray-200/60 dark:border-white/5 backdrop-blur-xl bg-white/80 dark:bg-background-dark/80 transition-colors duration-500">
                ...
                <div class="flex items-center justify-between h-16 lg:h-20">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-xl font-bold tracking-tight" style="font-family: 'Outfit', sans-serif">
                            <span class="text-gray-900 dark:text-white">Torneo</span><span class="text-primary">Hub</span>
                        </span>
                    </div>

                    <div class="hidden lg:flex items-center gap-8">
                        <a href="#features" class="text-sm text-gray-500 hover:text-gray-900 dark:text-white/60 dark:hover:text-white">Funciones</a>
                        <a href="#pricing" class="text-sm text-gray-500 hover:text-gray-900 dark:text-white/60 dark:hover:text-white">Precios</a>
                        <a href="#stats" class="text-sm text-gray-500 hover:text-gray-900 dark:text-white/60 dark:hover:text-white">Resultados</a>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div v-if="canLogin" class="hidden lg:flex items-center gap-3">
                            ...
                            <template v-else>
                                <Link
                                    :href="route('login')"
                                    class="px-5 py-2.5 text-sm font-medium text-gray-500 hover:text-gray-900 dark:text-white/70 dark:hover:text-white"
                                >
                                    Iniciar sesión
                                </Link>
                                <Link
                                    v-if="canRegister"
                                    :href="route('register')"
                                    class="px-5 py-2.5 text-sm font-semibold bg-primary hover:bg-primary-dark text-white rounded-xl hover:shadow-lg hover:shadow-primary/25"
                                >
                                    Prueba gratis
                                </Link>
                            </template>
                        ...
                        <button @click="isMenuOpen = !isMenuOpen" class="lg:hidden p-2 text-gray-500 dark:text-white/60 hover:text-gray-900 dark:hover:text-white">
            ...
            >
                <div v-if="isMenuOpen" class="lg:hidden border-t border-gray-200/60 dark:border-white/5 bg-white/95 dark:bg-background-dark/95 backdrop-blur-xl px-4 py-6 space-y-4">
        ...
        <section class="relative min-h-screen flex items-center pt-20">
            <div class="absolute inset-0 overflow-hidden">
                <div class="absolute top-0 left-0 right-0 h-full bg-gradient-to-b from-primary/[0.04] via-transparent to-transparent dark:from-transparent"></div>
                <div class="absolute top-1/4 -left-32 w-[500px] h-[500px] bg-primary/[0.06] dark:bg-primary/10 rounded-full blur-[120px]"></div>
                <div class="absolute bottom-1/4 -right-32 w-[400px] h-[400px] bg-emerald-300/[0.06] dark:bg-primary/5 rounded-full blur-[100px]"></div>
                <div
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-32">
                <div class="max-w-4xl mx-auto text-center">
                    ...
                    <h1 class="text-4xl sm:text-5xl lg:text-7xl font-extrabold tracking-tight leading-[1.1] mb-6 text-gray-900 dark:text-white" style="font-family: 'Outfit', sans-serif">
                        Tu liga,
                        <span class="relative inline-block">
                            ...
                            <span class="absolute bottom-2 left-0 right-0 h-3 bg-primary/15 dark:bg-primary/20 -skew-x-3 rounded"></span>
                        </span>
                        <br />como profesional
                    </h1>

                    <p class="text-lg sm:text-xl text-gray-500 dark:text-white/50 max-w-2xl mx-auto mb-10 leading-relaxed">
                        Gestiona torneos, equipos, jugadores, estadísticas y pagos desde una sola plataforma.
                        Automatiza lo tedioso y enfócate en lo que importa: <span class="text-gray-700 dark:text-white/80 font-medium">el fútbol.</span>
                    </p>
                    ...
                    <div class="mt-16 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-xs text-gray-400 dark:text-white/30">
            ...
            <div class="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-gray-300 dark:text-white/20">
                <span class="text-[10px] uppercase tracking-[0.2em]">Scroll</span>
                <div class="w-5 h-8 rounded-full border border-gray-300 dark:border-white/20 flex justify-center pt-1.5">
        ...
        <!-- ═══════════════ STATS ═══════════════ -->
        <section id="stats" class="relative py-20 border-y border-gray-200/60 dark:border-white/5 bg-white/50 dark:bg-transparent">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
                    <div v-for="stat in stats" :key="stat.label" class="text-center">
                        <div class="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-gray-900 dark:text-white mb-2" style="font-family: 'Outfit', sans-serif">
                            {{ stat.number }}
                        </div>
                        <div class="text-sm text-gray-400 dark:text-white/40">{{ stat.label }}</div>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section id="features" class="relative py-24 lg:py-32">
            <div class="absolute top-0 right-0 w-[600px] h-[600px] bg-primary/[0.04] dark:bg-primary/5 rounded-full blur-[150px]"></div>
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="max-w-2xl mx-auto text-center mb-16 lg:mb-20">
                    ...
                    <h2 class="mt-4 text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-gray-900 dark:text-white" style="font-family: 'Outfit', sans-serif">
                        Todo lo que necesitas,<br />
                        <span class="text-gray-300 dark:text-white/40">nada que no.</span>
                    </h2>
                </div>
                ...
                <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    ...
                    >
                        ...
                        <p class="text-sm text-gray-500 dark:text-white/40 leading-relaxed">{{ feature.desc }}</p>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section id="pricing" class="relative py-24 lg:py-32">
            <div class="absolute bottom-0 left-0 w-[600px] h-[600px] bg-primary/[0.04] dark:bg-primary/5 rounded-full blur-[150px]"></div>
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="max-w-2xl mx-auto text-center mb-12 lg:mb-16">
                    ...
                    <p class="mt-4 text-gray-400 dark:text-white/40">Empieza gratis. Escala cuando lo necesites.</p>

                    <div class="mt-8 inline-flex items-center p-1 rounded-2xl bg-gray-100 dark:bg-surface-dark border border-gray-200/80 dark:border-white/[0.06]">
                        <button
                        ...
                        >
                            Anual
                            <span class="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-bold">
                ...
                <div class="grid lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
                    <div
                        v-for="plan in plansData[activeTab]"
                        :key="plan.id"
                        :class="[
                    ...
                    >
                        ...
                        <div class="mb-6">
                            <h3 class="text-xl font-bold text-gray-900 dark:text-white">{{ plan.nombre }}</h3>
                            <p class="mt-1 text-sm text-gray-400 dark:text-white/40">{{ plan.descripcion }}</p>
                        </div>

                        <!-- Price - Versión corregida -->
                        <div class="mb-8">
                            <div class="flex items-baseline gap-1">
                                <!-- Mostrar precio anual completo cuando es anual -->
                                <template v-if="activeTab === 'anual'">
                                    <span class="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-white" style="font-family: 'Outfit', sans-serif">
                                        ${{ plan.precio.toLocaleString('es-MX') }}
                                    </span>
                                    <span class="text-gray-400 dark:text-white/30 text-sm">/año</span>
                                </template>
                                <template v-else>
                                    <span class="text-4xl sm:text-5xl font-extrabold text-gray-900 dark:text-white" style="font-family: 'Outfit', sans-serif">
                                        ${{ plan.precio }}
                                    </span>
                                    <span class="text-gray-400 dark:text-white/30 text-sm">/mes</span>
                                </template>
                            </div>
                            
                            <!-- Mostrar ahorro para plan anual -->
                            <div v-if="activeTab === 'anual' && plan.precioMensualEquivalente" class="mt-2 text-sm text-gray-400 dark:text-white/30">
                                <span class="line-through">${{ plan.precioOriginalMensual }}/mes</span>
                                <span class="text-primary ml-2">
                                    Equivale a ${{ plan.precioMensualEquivalente }}/mes
                                </span>
                            </div>
                            
                            <!-- Mostrar ahorro para plan mensual comparado con anual -->
                            <div v-if="activeTab === 'mensual' && plan.precio" class="mt-2 text-xs text-gray-400 dark:text-white/30">
                                <span class="text-primary">Ahorra 17%</span> con plan anual
                            </div>
                            
                            <div class="text-xs text-gray-300 dark:text-white/20 mt-1">MXN + IVA</div>
                        </div>
                        ...
                        <ul class="space-y-3">
                            <li v-for="feature in plan.features" :key="feature" class="flex items-start gap-3">
                                ...
                                <span class="text-sm text-gray-600 dark:text-white/60">{{ feature }}</span>
                            </li>
                            <li v-for="limit in plan.limitaciones" :key="limit" class="flex items-start gap-3">
                                <svg class="w-5 h-5 text-gray-200 dark:text-white/15 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                                </svg>
                                <span class="text-sm text-gray-300 dark:text-white/25">{{ limit }}</span>
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="mt-12 text-center">
                    <p class="text-sm text-gray-400 dark:text-white/30">
                        ...
                        <a href="#" class="text-primary hover:text-primary-dark underline underline-offset-4">Contáctanos</a>
        ...
        <section class="relative py-24 lg:py-32">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary/10 via-white to-white dark:from-primary/20 dark:via-surface-dark dark:to-surface-dark border border-primary/10 p-10 sm:p-16 lg:p-20">
                    ...
                    <div class="relative max-w-2xl">
                        <h2 class="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight leading-tight text-gray-900 dark:text-white" style="font-family: 'Outfit', sans-serif">
                            Lleva tu liga al<br />
                            <span class="text-primary">siguiente nivel</span>
                        </h2>
                        <p class="mt-5 text-lg text-gray-500 dark:text-white/40 max-w-lg">
        ...
        <!-- ═══════════════ FOOTER ═══════════════ -->
        <footer class="border-t border-gray-200/60 dark:border-white/5 py-12 lg:py-16 bg-white/30 dark:bg-transparent">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
                    <div class="sm:col-span-2 lg:col-span-1">
                        ...
                        <p class="text-sm text-gray-400 dark:text-white/30 leading-relaxed">
                            La plataforma SaaS para organizar y gestionar torneos de fútbol como un profesional.
                        </p>
                    </div>

                    <div>
                        <h4 class="text-xs font-semibold text-gray-500 dark:text-white/50 uppercase tracking-wider mb-4">Producto</h4>
                        <ul class="space-y-3">
                            <li><a href="#features" class="text-sm text-gray-400 dark:text-white/30 hover:text-gray-700 dark:hover:text-white">Funciones</a></li>
                            <li><a href="#pricing" class="text-sm text-gray-400 dark:text-white/30 hover:text-gray-700 dark:hover:text-white">Precios</a></li>
                            <li><a href="#" class="text-sm text-gray-400 dark:text-white/30 hover:text-gray-700 dark:hover:text-white">Integraciones</a></li>
                            <li><a href="#" class="text-sm text-gray-400 dark:text-white/30 hover:text-gray-700 dark:hover:text-white">API</a></li>
                        </ul>
                    </div>
                    <div>
                        <h4 class="text-xs font-semibold text-gray-500 dark:text-white/50 uppercase tracking-wider mb-4">Soporte</h4>
                        <ul class="space-y-3">
                            <li><a href="#" class="text-sm text-gray-400 dark:text-white/30 hover:text-gray-700 dark:hover:text-white">Centro de ayuda</a></li>
                            <li><a href="#" class="text-sm text-gray-400 dark:text-white/30 hover:text-gray-700 dark:hover:text-white">Documentación</a></li>
                            <li><a href="#" class="text-sm text-gray-400 dark:text-white/30 hover:text-gray-700 dark:hover:text-white">Contacto</a></li>
                        </ul>
                    </div>
                    <div>
                        <h4 class="text-xs font-semibold text-gray-500 dark:text-white/50 uppercase tracking-wider mb-4">Legal</h4>
                        <ul class="space-y-3">
                            <li><a href="#" class="text-sm text-gray-400 dark:text-white/30 hover:text-gray-700 dark:hover:text-white">Términos</a></li>
                            <li><a href="#" class="text-sm text-gray-400 dark:text-white/30 hover:text-gray-700 dark:hover:text-white">Privacidad</a></li>
                            <li><a href="#" class="text-sm text-gray-400 dark:text-white/30 hover:text-gray-700 dark:hover:text-white">Aviso legal</a></li>
                        </ul>
                    </div>
                </div>

                <div class="mt-12 pt-8 border-t border-gray-200/60 dark:border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <p class="text-xs text-gray-300 dark:text-white/20">&copy; {{ new Date().getFullYear() }} TorneoHub. Todos los derechos reservados.</p>
                    <div class="flex items-center gap-4">
                        <a href="#" class="text-gray-300 dark:text-white/20 hover:text-gray-500 dark:hover:text-white/50">

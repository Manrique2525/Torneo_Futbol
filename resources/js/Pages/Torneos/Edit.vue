...
const tipoOptions = Object.entries(props.constantes.tipos_torneo).map(([key, label]) => ({ label, value: key }));
const categoriaOptions = Object.entries(props.constantes.categorias).map(([key, label]) => ({ label, value: key }));
const ramaOptions = Object.entries(props.constantes.ramas).map(([key, label]) => ({ label, value: key }));
const estadoOptions = Object.entries(props.constantes.estados_torneo || {
    activo: 'Activo',
    finalizado: 'Finalizado',
    cancelado: 'Cancelado'
}).map(([key, label]) => ({ label, value: key }));

const selectedTipo = ref(tipoOptions.find(t => t.value === props.torneo.tipo) ?? null);
const selectedCategoria = ref(categoriaOptions.find(c => c.value === props.torneo.categoria) ?? null);
const selectedRama = ref(ramaOptions.find(r => r.value === props.torneo.rama) ?? null);
const selectedEstado = ref(estadoOptions.find(e => e.value === (props.torneo.estado ?? 'activo')) ?? null);
...
const form = useForm({
    ...
    baja_por_impago_automatica: props.torneo.baja_por_impago_automatica ?? false,
    max_jornadas_sin_pago: props.torneo.max_jornadas_sin_pago ?? null,
});
